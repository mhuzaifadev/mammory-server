...
class Mammogram(Resource):
    ...
    def post(self):
        ...
        if 'image' in request.files:
            ...
            thread = Thread(target=self.mammo_detect, args=(image,full_name,date_of_birth,email, queue))
            thread.start()
    ...
    def mammo_detect(self, image,full_name,date_of_birth,email,queue):
        
        report_type, lesion, risk_factor, message, sub_text = Image_Processing().mammogram_detect(image)
        # print(report_type, lesion, risk_factor, message, sub_text)
        processed_image_grid = Image_Processing().image_processing_grid(image,"Mammogram")
